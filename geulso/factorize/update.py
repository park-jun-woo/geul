...
class FrameTextUpdater:
    """JSON 파일에 frame_text와 definition을 추가하는 클래스"""
    
    def __init__(self, db_config: Dict[str, str]):
        """PostgreSQL 연결 초기화"""
        self.conn = psycopg2.connect(**db_config)
        self.frame_cache = {}  # (synset_id, frame_id) -> frame_text
        self.synset_cache = {}  # synset_id -> definition
        self.stats = {
            'total': 0,
            'updated': 0,
            'frame_not_found': 0,
    ...
    def load_caches(self):
        """모든 verb frame과 synset definition을 미리 메모리에 로드"""
        print("데이터베이스에서 데이터 로딩 중...")
        ...
        print(f"✓ {len(self.frame_cache):,}개의 verb frame 로드 완료")
        
        # Synset definitions 로드
        with self.conn.cursor() as cur:
            cur.execute("""
                ...
                WHERE pos = 'v'
            """)
            for synset_id, definition in cur.fetchall():
                self.synset_cache[synset_id] = definition if definition else ""
        print(f"✓ {len(self.synset_cache):,}개의 synset definition 로드 완료")
    ...
    def get_definition(self, synset_id: str) -> Optional[str]:
        """캐시에서 definition 조회"""
        return self.synset_cache.get(synset_id)
    
    def update_json_file(self, filepath: Path) -> bool:
        """JSON 파일에 frame_text와 definition 추가"""
        try:
            # JSON 파일 읽기
            ...
            # 이미 필요한 필드가 모두 있으면 건너뛰기
            has_definition = 'definition' in data
            has_frame_text = 'frame_text' in data
            
            if has_definition and has_frame_text:
            ...
            if not has_definition:
                definition = self.get_definition(synset_id)
                if definition is None:
                    self.stats['synset_not_found'] += 1
                    print(f"  ⚠ definition 없음: {synset_id}")
                else:
                    needs_update = True
            else:
                definition = None
            
            if not has_frame_text:
                frame_text = self.get_frame_text(synset_id, frame_id)
                if frame_text is None:
                    self.stats['frame_not_found'] += 1
            ...
            if needs_update:
                ...
                for key, value in data.items():
                    ...
                    # synset_id 바로 아래에 definition 추가
                    if key == 'synset_id' and definition is not None:
                        new_data['definition'] = definition
                    
                    # frame_id 바로 아래에 frame_text 추가
                    if key == 'frame_id' and frame_text is not None:
    ...
    def print_stats(self):
        ...
        print(f"frame_text 없음: {self.stats['frame_not_found']:,}개")
        print(f"definition 없음: {self.stats['synset_not_found']:,}개")
...
def main():
    print("="*60)
    print("GEUL JSON 파일에 frame_text와 definition 추가")
    print("="*60)
    
    updater = FrameTextUpdater(DB_CONFIG)
    
    try:
        # 모든 데이터를 먼저 캐시에 로드 (성능 향상)
        updater.load_caches()
