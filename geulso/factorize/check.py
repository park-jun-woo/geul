...
오류가 발견된 파일은 'OUTPUT_DIR'에 "errors" 필드가 추가된 채로 복사됩니다.
"""
...
SOURCE_DIR = Path("geulso/factorize/factorized")
...
def check_json_file(
    ...
    if isinstance(sememes, list):
        ...
        errors.append("`sememes`가 리스트가 아닙니다.")
...
def main():
    ...
    for file_path in tqdm(all_files, desc="파일 검증 중"):
        errors = check_json_file(file_path, validation_rules)
