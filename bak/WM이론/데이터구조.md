...
* GEUL 스트림/GPT 컨텍스트:
  ...
  * 덕분에:
    ...
    * 3항 엣지 기준
      ...
      * WMS: SIDX 4개 수준 (헤더+컨텍스트 포함 16워드)
      * GEUL 압축: TID 기준 6워드 정도까지 축소 가능.

---

요 정도가 **WMS 내부에서 World가 어떻게 구성되는지, 그리고 Entity/Context/Edge가 어떻게 배치되는지**에 대한 현재 정리본이라고 보면 될 것 같아.
(규칙 GEUL/서술 GEUL은 이 위에 올라가는 상위 레이어고, WMS는 일단 여기까지가 코어 구조.)
