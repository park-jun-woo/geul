...
`추론 GAT`가 "3과 5를 더해야 한다"는 결론에 도달하면, GDBMS에서 위 `코드 글오`를 검색합니다. 그리고 `x=3`, `y=5`를 입력값으로 GEUL 바이트코드 실행 엔진에 이 코드를 전달합니다. 실행 엔진이 `sum=8`이라는 결과를 반환하면, GAT는 이 결과를 받아 다음 추론 단계에 사용합니다.
