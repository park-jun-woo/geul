...
- **실행 코어 = 1블록 트랜스포머**
    ...
    여기서 **Q**는 쿼리에서 생성, **K,V**는 GDBMS 인덱스(사전 전처리), **M**은 필터·시간·확률 마스크.
...
3. **키/값 메모리(K,V) 준비**
    - GDBMS가 오브젝트별 **키 Ki∈RdK_i\in\mathbb{R}^d**, **값 ViV_i**, 메타(시간, 신뢰도, 태그) 제공.
...
- **캐시**: 인기 쿼리의 (Q,M)(Q,M) 캐시, 결과 Top-k/집계 캐시를 GDBMS와 공유.
