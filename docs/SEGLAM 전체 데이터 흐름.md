...
3.  **[쿼리 생성] → 쿼리 생성 GPT**
    ...
    * GDBMS에서 어떤 정보와 절차를 찾아야 할지에 대한 최적의 `쿼리 GEUL Stream`을 생성합니다.

4.  **[정보 인출] GDBMS**
    * `GDBMS`는 `쿼리 GEUL Stream`을 입력받아, 거대한 지식베이스에서 답변에 필요할 것으로 추측되는 `정보 GEUL Stream`(사실)과 `제어 GEUL Stream`(절차)을 초고속으로 인출하여 반환합니다.
...
1.  **[기록] 답변 → GDBMS**
    * `추론 GPT`가 생성한 `답변 GEUL Stream`은 사용자에게 전달되는 동시에, 하나의 성공적인 '경험'으로서 GDBMS에 다시 저장됩니다.

2.  **[성찰] GEUL-Agent 활성화**
    * 시스템 유휴 상태일 때, `GEUL-Agent`들이 활성화됩니다.
    * Agent는 GDBMS에 새로운 자연어 정보(뉴스 등)를 GEUL Stream으로 번역하여 업데이트하거나, 새로 기록된 '경험'들을 다시 분석합니다.

3.  **[개선] 지식 및 절차 업데이트**
    * Agent의 성찰을 통해 GDBMS의 자산이 개선됩니다.
