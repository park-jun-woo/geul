...
- GWMS는 이 Claim들의 집합 위에서 **합의(Consensus), 우도(Likelihood), Canon(정전)**을 계산할 수 있다.

GEUL은 “지도”를 그리는 언어이고,  
GWMS는 “지도들을 모아서 영토를 역추적하는 엔진”이다.

---

## 2. 진실의 4계층 – GEUL이 기대하는 세계 모델 구조

GEUL은 GWMS와 함께 다음과 같은 **진실 계층 구조**를 전제로 한다.
...
- 이는 GWMS 구현체(Go 코드, VM, 런타임) 내부에 존재하는 **절차(Procedure)**이다.
...
> GEUL의 출발점은 **항상 L3**이다.  
> 나머지는 GWMS와 상위 레이어(Reasoner)가 책임진다.
...
2. **GWMS 친화성**
   - MID 기반 의미정렬 ID → 내부 그래프와 자연스럽게 매핑
...
3. **표준·생태계 확장성**
   ...
   - Issuer 개념을 통한 타입 등록
   - “한 번 정의된 MID는 전 세계 어디서나 동일 의미”라는 약속
...
GWMS는 내부적으로 World State(노드, 엣지, 속성)를 관리하지만,  
GEUL은 **서술(발화, 문장, 사건)을 우선시**한다.

- 1단계: 자연어 → GEUL 서술 (Event, Triple, Verb Edge)
- 2단계: GEUL 서술 → GWMS World State(Triple Edge, Event6 등)로 정규화/추론
...
- GWMS/코드에게는 “빠르게 파싱 가능한 바이너리 스트림”

이라는 두 요구를 동시에 만족시키기 위한 타협이다.

### 4.5 원칙 5: TID는 “그/그녀/이것”이다

- MID(64-bit)는 **세계 전체에서 유일한 의미 ID**다.
...
1. **GEUL Standard (Core)**
   ...
   - 여기서 정의된 MID/Type은 **“전 우주 공용”**이라는 약속을 가진다.
...
- **GEUL**  
  - GWMS와 LLM, 외부 시스템 사이의 “연결 언어”  
  - 쿼리와 결과를 표현하는 **스트림 포맷**이자,  
    월드를 옮아가는 **이식 가능한 언어**.

### 7.2 GEUL은 GWMS 내부 포맷이 아니다 (원칙)

- GWMS 내부 저장 포맷(world.gwms)은 **메모리/디스크/캐시 최적화 구조**를 따른다.
  - SoA, 버킷, 인덱스, 압축 등.
- GEUL은 “전송/인터페이스 최적화 포맷”이다.
  - LLM, 기타 시스템, 다른 GWMS 구현체와 주고받기에 유리하게 설계.

둘은 **필요할 때 서로 변환**한다:

- `GWMS → GEUL` : 쿼리 결과를 LLM/외부로 보낼 때
- `GEUL → GWMS` : 자연어/LLM/입력 시스템이 만든 서술을 세계에 반영할 때
...
2. **월드층(L2/L1): GEUL 서술 → GWMS World**
   - 여러 서술을 모아,
   - 규칙과 충돌 여부를 확인하고,
...
3. **GEUL은 뉴로-심볼릭 인터페이스다.**  
   LLM과 GWMS 사이를 16-bit 워드 스트림으로 연결하는 중간 언어다.
...
이 철학 위에서 GEUL의 비트 구조, 스트림 포맷, GWMS 파일 포맷 설계가 결정된다.  
