# GEUL-설계철학.md  
*GEUL (General Embedding Universal Language) 설계 철학 v2.0*
...
### 3.3 GEUL은 내부 IR이다 (LLVM 비유)

**사용자에게 GEUL을 직접 노출하지 않는다.**
...
- **채택 장벽 = 0:** 사용자는 GEUL을 몰라도 됨
- **빅테크 인센티브 명확:** AI 추론 비용 절감이 직접적
- **네트워크 효과 자동 발생:** 모든 쿼리가 지식베이스에 기여
- **NIH 신드롬 무력화:** 각 기업이 자체 구현 가능
...
- 자연어 = 인간-인간 통신용
- AI 추론 = 구조화된 사고
- IR(Intermediate Representation) 필요
- **LLVM이 컴파일러에 한 일을 GEUL이 AI에 한다**
...
GEUL은 뻔한 질문의 비용을 극적으로 절감함으로써:

1. AI 추론 자원을 창의적 질문에 집중
2. 지식 접근 비용 하락
3. 창의성 투자 여력 확보
...
4. **GEUL은 내부 IR이다.**  
   사용자에게 노출되지 않는 내부 표현(LLVM IR처럼)으로 채택 장벽이 없다.

5. **서술층은 풍부하게, 월드층은 정규화해서.**  
   동사/한정사는 L3에서 최대한 살리고, L2/L1로 갈수록 필요한 만큼만 줄인다.

6. **Core/Extension/Custom의 3층 구조로 열린 표준을 지향한다.**  
   리얼월드·가상세계·기업·개인은 각각 자기 세계를 확장할 수 있고,  
   Core는 모두가 공유하는 공통 언어가 된다.

7. **GEUL은 필연이다.**  
   AI 추론 비용 절감, 자연어 탈피, 창의성 경제는 기술과 경제의 필연적 귀결이다.
...
|------|------|------|
| v1.0 | 2026-01 | 초안 |
| v2.0 | 2026-01-30 | 경제적 필연성, 내부 IR 비유, 창의성 경제 추가 |
