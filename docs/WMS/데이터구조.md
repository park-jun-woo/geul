오케이, 지금까지 나온 걸 **GDBMS 내부 관점**에서만 싹 정리해볼게.
(월드/엔티티/컨텍스트/엣지/ID 중심으로)
...
* 한 EID에 대해, 시대/역할/상태에 따라 **여러 MID 프로필**이 달라질 수 있음.

  * 예: 머스크가 나중에 대통령 되면 대통령 상태용 MID 추가.
...
* GEUL/GPT 입출력용:
  ...
  * 쿼리 결과에서 등장한 MID들만 모아
  * `TID → MID` 맵핑 만들어서 1워드(16bit)로 압축.
* GDBMS ↔ GEUL 브릿지에서만 사용.
...
* 로컬 World 전용 동사도 MID 상위 비트로 scope 구분해서 추가.
...
1. **Entity 영역**
   ...
   * MID 상위 4~8비트로 버킷킹된 `EntityBucket[]`
...
3. **보조 인덱스**
   ...
   * MID → EntityBucket 위치
   * prop_id → Property 메타/Verb MID/QuantitySpec 등
...
* GDBMS 내부:
  ...
  * **항상 MID 64bit** 기준으로 노드/엣지 관리.
* GEUL 스트림/GPT 컨텍스트:

  * 상단 TIDMAP에서 `TID (16bit) → MID` 매핑
  * Node/Edge는 전부 TID로 서로를 참조
  * 덕분에:

    * 3항 엣지 기준

      * GDBMS: MID 4개 수준 (헤더+컨텍스트 포함 16워드)
      * GEUL 압축: TID 기준 6워드 정도까지 축소 가능.

---

요 정도가 **GDBMS 내부에서 World가 어떻게 구성되는지, 그리고 Entity/Context/Edge가 어떻게 배치되는지**에 대한 현재 정리본이라고 보면 될 것 같아.
(규칙 GEUL/서술 GEUL은 이 위에 올라가는 상위 레이어고, GDBMS는 일단 여기까지가 코어 구조.)
