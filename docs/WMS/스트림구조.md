...
### 1) MID (Meaning ID, 64bit, 4워드)

* GDBMS의 **기본 ID**.
* 상위 56bit: 의미정렬 비트 (도메인/역할/세계선/시대/POV 등).
* 하위 8bit: 충돌 방지/여유.
* 사람/조직/장소/세계/컨텍스트/세계관(POV)… 전부 MID로 표현.
* 장점:

  * 대통령/사업가/지구667/특정 시대/특정 세계선 같은 걸 **비트마스크로 고속 필터** 가능.
  * GDBMS 검색은 전부 MID 기반으로 설계.

### 2) CTXMID (Context = 세계+시점+POV)

* Context도 결국 하나의 **개체 노드 MID**.
* 안에 논리적으로 포함되는 것:
...
* 모든 사실/엣지는 **“어떤 Context에서 참이냐”**를 CTXMID로 지정.

### 3) EID (Existence ID)

* 동일성(“이 사람은 이 사람이다”)을 위한 ID.
* 하나의 EID가 시간/역할에 따라 여러 MID 프로필(상태)을 가질 수 있음.
* 예: 머스크가 나중에 대통령이 되면 “대통령 상태 MID”를 새로 부여.
...
* 스트림 상단에 `TIDMAP`:
  ...
  0x0001 -> MID(아이언맨)
  0x0002 -> MID(지구667)
  0x0003 -> MID(CTX_지구667_2000_마블공식)
  ```
...
* `Value/Object` (1bit):
  ...
  * 0 = 값 계열 (하지만 GDBMS에서는 따로 안 씀)
  * 1 = 개체(Object)
* `L-Payload` / `ObjectType` 등은 스트림용 스펙이고,
  GDBMS 내부에선 더 조밀하게 재패킹 가능.
...
* GDBMS 내부에선 각 슬롯이 전부 **MID or 값 64bit**.
...
* Wikidata Triple ⇒ GEUL Triple Edge로 매핑:
  ...
  * **GDBMS 내부**:
    ...
    * `obj_mid` 또는 `value64`
  * `keyframe_mid` 같은 건 Context/MID 쪽으로 흡수 (Context 축에서 해결).
...
* 대통령 관련 사건처럼 **역할/세계선·시대 필터링**은 MID 비트마스크로 처리.
...
* “지구667의 2000년”, “지구667의 Scene_45”,
  ...
  * world + time + pov + modality 조합 → 하나의 CTXMID.
...
* 스트림 스펙에 있던 “물리량 노드” 구분은 **GDBMS 레벨에선 제거**.
* 대신:
...
* GDBMS ↔ GPT 변환 시:
  ...
  * 쿼리 결과에서 등장한 MID들을 순차 scan하면서
    새 MID마다 TID auto increment 부여.

---

## 7. 전체 철학 요약

1. **의미/그래프/쿼리는 전부 GDBMS + MID 세계에서 해결.**

   * 64bit MID에 의미를 정렬해두고,
   * Context(세계+시점+POV)도 MID로,
...
3. **Node/Edge는 GDBMS에서 4워드 고정.**

   * 헤더는 타입/플래그,
   * 슬롯은 MID/값,
   * 물리량은 QuantitySpec 노드 + 64bit value로 관리.

4. **복잡한 건 MID/Context/Property/QuantitySpec에 몰고,
   TID는 그냥 번호표로 단순하게.**

이 정도가 지금까지 우리가 같이 쌓아올린 GDBMS+GEUL 설계의 골격이야.
