...
| 항목 | 값 |
|------|-----|
| Standard | `0 000 000` (7비트) |
| Proposal | `1100 000 000` (10비트) |
| 1st 워드 나머지 | 6비트 (Type 4 + Payload 2) |

**설계 근거:** `0 000 000` (모든 비트 0)은 "가장 근본적인 메타"를 의미.
...
```
16비트 TID 스트림 시작:
  [1100 000 000] [0000] [00] = 0xC000

32비트 TID 스트림 시작:
  [1100 000 000] [0000] [01] = 0xC001
```
...
2026-01-30 12:00:00 UTC (32비트):
  ...
  1st: [1100 000 000] [0010] [00] = 0xC008
  2nd: [0x6978]
  3rd: [0xC900]
...
```
생성자 미상:
  1st: [1100 000 000] [0100] [00] = 0xC010

총: 1워드

생성자: Wikidata Q5 (human):
  1st: [1100 000 000] [0100] [01] = 0xC011
...
```
버전 v1.0:
  1st: [1100 000 000] [0101] [00] = 0xC014
  2nd: [0x0100]
...
def parse_meta_node(data: bytes) -> dict:
    ...
    # Prefix 확인
    prefix = word1 >> 6
    assert prefix == 0b1100000000, "Not Meta Node"
...
### 9.1 Prefix `0 000 000` 선택 이유

- **의미적 적합성:** 모든 비트 0 = "가장 근본적인" 메타 정보
- **파싱 우선순위:** 스트림 시작점에서 첫 번째로 만남
- **직관성:** "0으로 시작하면 메타"
