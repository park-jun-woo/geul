...
이 비트들은 Context 노드의 타입과 메타정보를 compact하게 표현하는 용도.
...
Unit Type가 “3D Position”이라면, 뒤에 x,y,z 값을 3×64비트로 붙이는 식.
...
실제 항(Ctx, Subj, Obj …) 값들은 GWMS 내부에서 별도 MID/TID 필드로 관리하며,
...
Extension 영역은 네임스페이스/기관/자유/차세대용 메타 영역으로 사용한다.
...
bit6=1인 경우, 실제 값 표현은 타입별 별도 스펙을 둔다.
...
이 문서는 64비트 GEUL Word/MID의 상위 비트 구조에 대한 고정 스펙이다.
