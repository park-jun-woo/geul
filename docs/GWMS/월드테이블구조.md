...
GEUL WORLD ENGINE / GWMS v0.x  
작성자: (설계 초안)  
목적: World 단위로 **MODEL / RULE / BOOK** 테이블을 어떻게 나누고,  
...
- `MODEL` : **월드 상태 (State)** 를 담는 테이블들
- `RULE`  : **법칙 / 규칙 (Rules)** 을 담는 테이블들
- `BOOK`  : **서술 (Narrative / Log / Report)** 을 담는 테이블들

> 핵심 아이디어  
> - **MODEL/RULE** 은 인간 인식 구조에 맞춰 **타임라인 기반** 설계를 채택  
...
World {
  ...
  timeline: {
    ...
    keyframe_model_table_id: TableId // 타임라인 Keyframe 엔티티가 들어있는 MODEL 테이블
  }
}
````
...
* **한 World 안에 MODEL / RULE / BOOK 테이블을 여러 개 둘 수 있음**

  * MODEL: `core_model`, `character_model`, `erp_model` …
...
* MODEL / RULE / BOOK 어디서든 이 Keyframe을 참조할 수 있음
...
MODEL 테이블은 결국 **노드(Node)와 엣지(Edge)**의 집합이다.
...
ModelEdge {
  id: EdgeId
  ctx: MID             // Context (world, pov, modality)
  subj: MID
...
* **Timeline Keyframe은 MODEL 테이블 안의 특수 노드 타입**
...
RuleEdge {
  ...
  trigger_link: MID?             // "동사ID" 등으로의 링크

  // 적용 범위
  scope_world: WorldRef?         // 특정 월드 또는 서브월드
  scope_ctx: MID?                // Context (POV, Modal 등)
  scope_filter: {...}            // Entity 타입, 태그 등

  // 효과 (Effect)
  effect_ref: MID?               // "벌점 부과 규칙", "HP -10" 같은 액션 노드
...
BookEntry {
  ...
  // 월드와의 연결 (선택)
  related_entities: [MID...]        // 이 서술이 언급하는 엔티티들
  related_events: [MID...]          // 이미 Model에 있는 Event 노드들
}
```
...
1. **World** 는 MODEL / RULE / BOOK 테이블들의 컨테이너다.
2. **MODEL 테이블**
...
```text
BOOK  →  (SCULPT / 파서)  →  MODEL / RULE (Timeline 기반)
MODEL / RULE  →  (WRENDER / 질의) →  서술/보고/설명 생성
```
