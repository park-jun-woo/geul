...
작성자: (설계 초안)  
목적: World 단위로 **CORE / RULE / BOOK** 테이블을 어떻게 나누고,  
...
- `CORE` : **월드 상태 (State)** 를 담는 테이블들
- `RULE`  : **법칙 / 규칙 (Rules)** 을 담는 테이블들
- `BOOK`  : **서술 (Narrative / Log / Report)** 을 담는 테이블들

> 핵심 아이디어  
> - **CORE/RULE** 은 인간 인식 구조에 맞춰 **타임라인 기반** 설계를 채택  
...
World {
  ...
  timeline: {
    ...
    keyframe_model_table_id: TableId // 타임라인 Keyframe 엔티티가 들어있는 CORE 테이블
  }
}
````
...
* **한 World 안에 CORE / RULE / BOOK 테이블을 여러 개 둘 수 있음**

  * CORE: `core_model`, `character_model`, `erp_model` …
...
* CORE / RULE / BOOK 어디서든 이 Keyframe을 참조할 수 있음
...
CORE 테이블은 결국 **노드(Node)와 엣지(Edge)**의 집합이다.
...
* **Timeline Keyframe은 CORE 테이블 안의 특수 노드 타입**
...
1. **World** 는 CORE / RULE / BOOK 테이블들의 컨테이너다.
2. **CORE 테이블**
...
```text
BOOK  →  (SCULPT / 파서)  →  CORE / RULE (Timeline 기반)
CORE / RULE  →  (WRENDER / 질의) →  서술/보고/설명 생성
```
