# SLT (Shallow Logic Transformer)

## 1. 개요: SLT란 무엇인가

SLT(Shallow Logic Transformer)는 글월/GEUL 기반 글DBMS/GWMS 내부에 탑재되는 **초경량 논리 신경망 모듈**이다.
...
- 쿼리 시
  ...
  - 또는 `WHERE SCORE(...) >= threshold`

### 3.2 SLT가 하지 않는 일
- “이 설정은 최종적으로 맞다/틀리다”라는 **확정 판정**
- 세계관 규칙을 위반하는 **강제적 상태 수정**
- 자연어 생성

SLT는 어디까지나 **“취향/맥락/논리 적합도 필터”**이며,  
최종 결정권은 심볼릭 제약 + 상위 LLM + 작가에게 있다.

---

## 4. 쿼리 파이프라인에서의 사용 방식

SLT는 SQL/쿼리 레벨에서 **스칼라 함수**처럼 사용된다.
...
* 초보 사용자: `ORDER BY SCORE(...) LIMIT 10`만 써도 되고
...
초경량 스펙(2-layer, d=256, L≈256) 기준:

* SLT 1회 추론은 sub-GFLOPs 수준
* 후보 1,000건 × SLT 2–3개

  * H100 기준 수 ms~수십 ms 사이
...
는 접근은 성능·지연 시간 측면 모두 현실적인 설계이다.

---

## 7. 기본 SLT vs 커스텀 SLT

GWMS는 크게 두 종류의 SLT를 상정한다.

### 7.1 기본 논리 SLT 세트 (Built-in)
...
SLT를 여러 개 쌓아두면, GWMS는 단순한 저장소를 넘어
...
1. 심볼릭/SIMD
   ...
   * `Category=VEHICLE`으로 후보 수천 개 생성
...
```text
Score = 1.0·Speed + 0.8·Durability + 0.5·Heroic
```
...
* “위험하지만 아름다운 존재”
  ...
  * `Danger_SLT + Beauty_SLT`
* “조금 슬프면서 장엄한 장면 후보”

  * `Sadness_SLT + Majestic_SLT`
...
GWMS 내부에 탑재되는 **초경량 논리 스코어링 모듈**이다.
