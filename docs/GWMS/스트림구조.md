...
* `Value/Object` (1bit):
  ...
  * 0 = 값 계열 (하지만 GWMS에서는 따로 안 씀)
  * 1 = 개체(Object)
* `L-Payload` / `ObjectType` 등은 스트림용 스펙이고,
  GWMS 내부에선 더 조밀하게 재패킹 가능.
...
* GWMS 내부에선 각 슬롯이 전부 **MID or 값 64bit**.
...
* 스트림 스펙에 있던 “물리량 노드” 구분은 **GWMS 레벨에선 제거**.
* 대신:
...
1. **의미/그래프/쿼리는 전부 GWMS + MID 세계에서 해결.**
...
이 정도가 지금까지 우리가 같이 쌓아올린 GWMS+GEUL 설계의 골격이야.
