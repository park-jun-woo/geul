...
- 역할:
  - **World** 단위로 CORE / RULE / BOOK 테이블을 관리.
  - GEUL 스트림으로 입출력.
...
- 예:
  ...
  - `World: AcmeCorp_ERP`
- 구성:
  - 여러 개의 **Table** (CORE / RULE / BOOK)을 가질 수 있음.  
  - 자체 **타임라인 설정** 보유  
    - `time_zone`, `calendar`  
    - `keyframe_model_table_id`: 타임라인 Keyframe 엔티티가 들어 있는 CORE 테이블.  
...
### 2.1 L1 – World Model Layer (CORE Layer)
...
### 3.2 MID (Meaning ID / Meaning Identifier)
...
- 역할:
  - 한 스트림 안에서 자주 참조되는 MID를 **짧게 압축**해서 쓰기 위한 핸들.
...
- 한 World 안에 여러 CORE 테이블이 존재할 수 있다 (예: `core_model`, `character_model` 등).  
...
- SQL/쿼리 레벨에서 **스칼라 함수**처럼 사용:
  - `ORDER BY SCORE(query, resource) DESC LIMIT K`
  - 또는 `WHERE SCORE(query, resource) >= threshold` 형태.  
