...
## 용어
1. **GEUL(General Embeding Unified Language)**: 범용 임베딩 통합 언어
...
1.  **HPA(Hypothesis-Driven Parsing Algorithm) Pruning 파이프라인 실증**: MRS 파싱 결과물에서 추출한 명사 및 동사 노드의 의미 후보(Wikidata Q-ID, WordNet synset_id)를 `prune_word_prompt.txt` 템플릿과 `ollama gpt-oss:20b` 모델을 이용하여 문맥에 맞게 Pruning하는 `mrs.py` 스크립트를 완성하고, 고전적인 중의성 문장으로 성능을 테스트한다.
...
### **2. Factorize 데이터 정제 방식 고찰**

Gemini가 생성한 상세 의미 분석 자료(JSON)의 환각 오류(`invalid.sememes.json` 등)를 검토했다.

* **문제 식별**: `gpt-oss:20b` 모델은 `find.py`가 제공한 후보군이 부적절할 경우, `abacinate` 예시처럼 **문맥과 무관한 엉뚱한 답을 창작**하거나, `abstain` 예시처럼 **일관성 없는 추론**을 하는 경향을 보였다.
* **프롬프트 개선**: 이 문제를 해결하기 위해, `validate_prompt.txt`에 **"99% 확신이 없으면 'NO_CANDIDATE'를 출력하라"**는 규칙과, **"후보군에 답이 없으면 스스로 유효한 답을 제안하라"**는 규칙을 순차적으로 적용하도록 프롬프트를 개선했다.
* **결과**: 개선된 프롬프트는 `abacinate`의 정답(`sheet_metal.n.01`)을 후보군에서 정확히 찾아냈고, `abscond`처럼 후보군이 없는 경우에도 `companion.n.01`이라는 훌륭한 대안을 스스로 제시하는 등 훨씬 더 안정적인 성능을 보였다.
