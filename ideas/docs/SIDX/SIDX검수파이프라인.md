...
[필드 정의]
header (필수): noun, verb, edge, doc_meta, source
...
[출력 형식]
{
  "header": "...",       "header_conf": "high|medium|low",
...
```
[시스템 — 문서 전체에서 모든 엔티티 추출]

다음 문서를 분석하여 세 가지 레이어로 태그하시오.
각 레이어에서 해당하는 모든 엔티티를 빠짐없이 추출할 것.
각 엔티티에 대해 확신도를 표시할 것.

Layer 1 — 문서 메타: 이 문서의 유형
Layer 2 — 출처: 이 문서의 출처/작성자
Layer 3 — 콘텐츠: 문서에 언급된 모든 엔티티와 동작
...
{
  "doc_meta": {
    "header": "doc_meta", "type": "document", "subtype": "news",
    "region": "east_asia", "era": "current",
    "conf": "high"
  },
  "source": {
    "header": "source", "type": "organization", "subtype": "media",
    "region": "east_asia", "entity": "조선일보", "qid": "Q482453",
    "conf": "high"
  },
  "entities": [
    {
      "header": "noun", "type": "person", "subtype": "business",
      "region": "east_asia", "era": "current",
      "entity": "이재용", "qid": "Q484523",
      "conf": "high"
    },
    {
      "header": "noun", "type": "organization", "subtype": "company",
      "region": "east_asia", "era": "current",
    ...
    {
      "header": "noun", "type": "organization", "subtype": "restaurant",
      "region": "east_asia", "era": "current",
      "entity": "깐부치킨", "qid": null,
      "conf": "medium"
    },
    {
      "header": "verb", "type": "event", "subtype": "meeting",
      "region": "east_asia", "era": "current",
      "entity": "회동", "qid": null,
      "conf": "high"
    }
...
func validateField(value string, validValues []string) error {
    if !slices.Contains(validValues, value) {
        return fmt.Errorf(
            "invalid value %q, valid: %v", value, validValues,
        )
    }
    return nil
}
...
```go
func validateConsistency(tag Tag, cb Codebook) error {
    validSubs := cb.SubtypesFor(tag.Type)
    if !slices.Contains(validSubs, tag.Subtype) {
        return fmt.Errorf(
            "subtype %q invalid for type %q, valid: %v",
            tag.Subtype, tag.Type, validSubs,
        )
    }
    return nil
}
...
```go
func applyConstraints(tag *Tag, constraints []Constraint) []string {
        ...
        if c.Matches(tag) {
            ...
            if !before.Equals(tag) {
                corrections = append(corrections,
                    fmt.Sprintf("%s: %v → %v", c.Name, before, tag))
            }
        }
    }
    return corrections
...
```
헤더 5비트와 필드 내용이 일관되는가?

입력: {"header": "verb", "type": "person", "subtype": "military"}

검사:
  header=verb(동사)인데 type=person(명사적 타입) → 불일치
  동사 헤더의 유효 type: [event, action, decision, ...]
...
결과: REJECT — header, subtype, region, era 누락
```
...
```go
func validateQID(tag Tag, wikidata WikidataDB) error {
    if tag.QID == "" || tag.QID == "null" {
        return nil  // Q-ID 없는 엔티티 허용
    }
    label := wikidata.GetLabel(tag.QID)
    if label == "" {
        return fmt.Errorf("Q-ID %s does not exist", tag.QID)
    }
...
소형 LLM: {"type": "person", "subtype": "military"}
          (header, region, era 누락)
VALID: ❌ 필수 필드 누락
재시도: "모든 필드를 빠짐없이 채우시오: header, type, subtype, region, era"
...
```go
func detectAnomalies(entries []Entry, cb Codebook) []Alert {
    buckets := countBuckets(entries)
    expected := len(entries) / cb.TotalBuckets()
