...
class KnowledgeLinker:
    ...
    def search_wordnet(self, lemma, pos=None):
        ...
        Returns minimal information: synset ID and definition only
        """
        pos_map = {
            'n': wn.NOUN,
            'v': wn.VERB,
        ...
        try:
            ...
            for synset in synsets:
                # Only synset name and definition - no examples, no lemmas
                candidates.append({
                    'synset': synset.name(),
...
def save_all_mrs_to_yaml(mrs_list: list, noun_nodes: list, verb_nodes: list, 
    ...
    output_data = {
        'metadata': {
            ...
            'note': 'Pruned: Wikidata for nouns only, empty descriptions removed, WordNet examples/lemmas removed, simplemrs removed'
        },
        'parses': parses,
        'nodes': {
            'nouns': noun_nodes,
